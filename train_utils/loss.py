...
class SCELoss(torch.nn.Module):
    def __init__(self, alpha=0.5, beta=0.5, num_classes=2):
        ...
        self.cross_entropy = torch.nn.CrossEntropyLoss(weight=torch.tensor([0.3, 0.7]).to('cuda'))
...
# cri = MeanAbsoluteError(num_classes =3)
# pre = torch.randn(4,3).cuda()
