...
def CountSketchFn_forward(h, s, output_size, x, force_cpu_scatter_add=False):
    ...
    if force_cpu_scatter_add:
        ...
        out = x.new(*out_size).zero_().to(device)
        return out.scatter_add_(-1, h.to(device), xs.to(device)).to(device)
...
def CountSketchFn_backward(h, s, x_size, grad_output):
    ...
    grad_x = grad_output.gather(-1, h.to(device)).to(device)
    grad_x = grad_x.to(device) * s.to(device)
    return grad_x
...
def ComplexMultiply_forward(X_re, X_im, Y_re, Y_im):
    ...
    # Z_re = torch.addcmul(X_re*Y_re, X_im, Y_im, -1)
    # Z_im = torch.addcmul(X_re*Y_im, X_im, Y_re,  1)
    return Z_re,Z_im
...
class CompactBilinearPooling(nn.Module):
    ...
    def __init__(self, input1_size, input2_size, output_size, h1 = None, s1 = None, h2 = None, s2 = None, force_cpu_scatter_add=False):
        ...
        self.output_size = output_size
        self.force_cpu_scatter_add = False
